...
public class Board {
    ...
    public boolean play(Player player, Move move) throws InvalidMoveException {
        ...
        if(matrix[i][j] != ' '){
            throw new InvalidMoveException("Esta jogada já foi feita, tente novamente!");
        }


        matrix[i][j] = player.getSymbol();

        return checkRows(player) || checkCols(player) || checkFirstDiag(player) || checkSecondDiag(player);
    }
    ...
    private boolean checkFirstDiag(Player player){
        char symbol = player.getSymbol();
